...
func github.com/phcbarros/imersao-full-cycle-venda-ingressos/golang vent_Validate(t *testing.T) {
	event := &Event{
		Name:     "",
...
func github.com/phcbarros/imersao-full-cycle-venda-ingressos/golang vent_AddSpot(t *testing.T) {
	event, err := NewEvent("Concert", "Stadium", "Music Inc.", RatingLivre, time.Now().Add(24*time.Hour), 100, 50.0, "http://x.jpg", 1)
	assert.Nil(t, err)
	assert.NotNil(t, event)
